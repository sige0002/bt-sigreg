...
無効化にはデータ転送待ちが増えて学習速度が下がる可能性がある。一般的なデメリットと、このGB10で速度差をまだ確定していない点は[学習手順のpin-memory説明](../reference/DATASET_RECIPES.ja.md#pin-memory-tradeoffs)に記載した。本試験のwall timeや異なる同時稼働区間を、そのまま速度比較として使わない。
