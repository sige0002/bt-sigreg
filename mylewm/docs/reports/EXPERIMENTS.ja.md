...
PushT更新日：2026-09-19。他項目の棚卸しは2026-09-16時点。[比較手順](../reference/COMPARISON_PROTOCOL.ja.md)の棚卸し用一覧。全ファイルを網羅した自動台帳ではない。下表の「記録」は文書の実施結果、「今回確認」はファイル・完了記録の読み取りであり、再評価・重みの全読込・データ全量hash検証ではない。稼働状態はこの表から推測しない。

## PushT：世界モデルの制御比較

| 対象 | 既存の証拠 | 比較に使える範囲／不足 |
|---|---|---|
| Raw／BT clip90・各140k | 両学習と最終評価完了。最終4探索seed平均はRaw89.5%／BT92.0%。2万〜14万の推移・条件・証拠は[結果まとめ](PUSHT_RAW_BT_140K.ja.md) | 基本制御性能に一貫した劣化は観測せず。単一学習seed・固定50ケース。マルチタスク優位性や一般的非劣化の証明ではない |
