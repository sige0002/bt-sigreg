...
| PushT | 旧BTの参考比較と、同条件Raw／BTの20k・40k・60k固定ケース比較を確認。60kではBTの上回りは続かなかった | 最終予算・複数seedでのRaw／TCに対する一般的優位性 |
...
- PushTは同じ学習経路・予算・物理初期状態のRaw／BT途中比較を確認済み。最終予算と複数seedの比較が必要。過去Raw70k／BT70kの条件差を残したまま主比較に昇格させない。
