...
PushT更新日：2026-09-19。現在の主方針は**CEMを固定してBT-SIGRegの比較評価を蓄積すること**。BCやCEM改良の診断は補助資料として扱う。[比較手順](../reference/COMPARISON_PROTOCOL.ja.md)・[既存実験一覧](EXPERIMENTS.ja.md)から進めてください。
...
| PushT | 同条件Raw／BTの14万更新学習・最終4探索seed評価まで完了。今回の条件では基本制御性能に一貫した劣化は観測せず。[結果](PUSHT_RAW_BT_140K.ja.md) | 一般的非劣化・統計的非劣性、マルチタスクでの優位性、独立学習seed間の再現性 |
...
- PushTは同じ学習経路・予算・物理初期状態で最終14万まで比較済み。探索seedの反復と独立学習seedを区別し、一般化を述べるには後者の比較が必要。Tの追加計算コストの分離測定も未実施。過去Raw70k／BT70kの条件差を残したまま主比較に昇格させない。
...
個別の数値・実行条件・検証時点は[実験一覧](EXPERIMENTS.ja.md)と各レポートに記録します。
