...
一覧整理：2026-09-16。研究仕様と現行手順は[文書一覧](../README.md)を参照。各レポートの実験日・検証範囲は本文に記載しています。

現在の研究方針は[比較手順](../reference/COMPARISON_PROTOCOL.ja.md)、再利用できる結果と不足は[実験一覧](EXPERIMENTS.ja.md)を参照してください。このページは個別レポートの索引です。
...
| レポート | 内容 |
|---|---|
| [TC-LeWM条件と再学習](TCLEWM_ALIGNMENT.ja.md) | 再生成データ、採用した構成、既存BT10k・補助BC40kの固定ジョブ |
...
未来予測器を使わない経路です。生成行動の教師誤差と、環境での成功率を区別します。
...
| [BC実生成行動の診断](LIBERO_BC_ACTION_DIAGNOSTIC.ja.md) | 全388デモ・3時刻・3生成、5k／25k／35kの教師行動誤差と汎化 |
| [再生成データBCの5k／25k評価](LIBERO_BC_OPENVLA_EVALUATION.ja.md) | 256px対応・画像監査、5k／25k／34k環境評価の起動・停止記録 |
