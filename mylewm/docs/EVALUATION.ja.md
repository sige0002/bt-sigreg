...
初期状態とケースを固定し、探索だけの乱数を変えるには`--seed 42 --cem-seed 7`を指定します。`--cem-seed`は既存solver専用の乱数生成器へ渡し、省略時は従来どおり`--seed`と同じ値になります。モデル・CEMの探索量は変更しません。

同じCEM seedごとにRaw／BTを対にして評価し、全seedでケース・初期状態／Goalのhashが一致することを確認します。各出力先は未使用の別名にします。seed間で成功率の平均・範囲と方式差を報告し、同じ50ケースを3 seedで試しても独立な150ケースとは数えません。これは探索の乱数による変動であり、学習seed間の変動ではありません。
