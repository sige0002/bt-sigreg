...
その後の明示依頼による[対照試験](reports/GB10_MEMORY_CONTROLS.ja.md)では、PushT単独・LIBERO単独は観測中に失敗せず、両者の併走で同じOOMが再現しました。PushT側の約16GiBの固定host予約を無効にすると観測中は成功し、有効に戻すと再発しました。最初の併走試験ではLIBEROだけの停止でも回復しました。評価自体の必要量に加え、学習側の固定メモリと新規CUDA初期化の可否も考慮します。既存runの再開条件を変更する許可や、長時間併走の保証を意味しません。
