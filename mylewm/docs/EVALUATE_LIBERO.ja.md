...
bash mylewm/run_libero.sh mylewm/tools/evaluate_libero.py --checkpoint CHECKPOINT --manifest output/manifests/libero10/manifest.json --render-audit-dir output/libero10/render_audit --output FRESH_OUTPUT_DIRECTORY
```
...
評価には全10タスクの画像監査が必要。既定の監査先は `output/libero10/render_audit/task_N/report.json`。新規監査は別出力先に保存し、評価の `--render-audit-dir` で指定する。学習済み共有LIBEROモデルの制御性能は未検証であり、BC成績とCEM成績を混ぜない。


新規の画像監査・評価出力は `output/libero10/` 以下の未使用ディレクトリへ保存してください。環境の構築・画像監査・制御評価は別工程です。各CLIの `--help` で必須引数を確認し、PushTの重みをLIBEROへ渡さないでください。

単独の旧環境smokeツールは撤去しました。画像監査が環境の起動と記録状態の再生を確認し、評価側が所定の初期状態を検査します。最初の監査例は1タスクだけです。実評価の前に、`--task-index 0`～`9`を各 `output/libero10/render_audit/task_N`（未使用パス）へ出力してください。旧監査を使う場合は旧ディレクトリを明示し、保存内容を新しい場所・版に合わせて書き換えないでください。
