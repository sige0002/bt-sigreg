...
1. [比較評価の手順](COMPARISON_PROTOCOL.ja.md)：何を固定し、どの順番でRaw／TC／BTを比較するか。
2. [既存実験の一覧](EXPERIMENTS.ja.md)：PushT・LIBERO・公式LeWMの結果をどう使えるか、不足は何か。
3. [検証状況](VALIDATION.ja.md)：現在言える結論と、未完了の主比較。

CEM制御を主な能力評価、BCを視覚表現の補助評価として扱います。実験を始める前に比較仕様を決め、実行操作は下のガイドを参照してください。
...
| 4 | [評価](EVALUATION.ja.md) | 決めた比較条件で評価を実行し、結果を確認できる |
...
検証の古い時系列は[検証履歴](reference/VALIDATION_HISTORY_20260915.ja.md)に保持しています。現行ガイドへ日付順の進捗を積み重ねず、詳細は`reports/`、その一覧と現在地は上の入口へ反映します。
