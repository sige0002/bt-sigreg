...
cd "$EVAL_ROOT"
exec uv run --no-sync python - "$EVAL_ROOT" "$@" <<'PY'
import argparse
import hashlib
import json
import os
...
plan = {'checkpoint': str(checkpoint), 'dataset': str(dataset), 'output': str(output), 'cases': len(cases),
        ...
        'launcher_sha256': hashlib.sha256((root/'scripts/evaluate_pusht.sh').read_bytes()).hexdigest(),
        'command': command}
...
from mylewm.evaluation.contract import validate_result, protocol
