...
from mylewm.training.loop import preprocess
...
def audit_model(checkpoint, cases, dataset_path, manifest, device):
    ...
    return {'checkpoint': str(checkpoint), 'checkpoint_sha256': file_sha256(checkpoint),
...
def main():
    ...
    expected_hashes = {'bt': file_sha256(args.bt_checkpoint), 'official': file_sha256(args.official_checkpoint)}
    ...
    for result in (bt_result, official_result):
        if result.get('provenance', {}).get('manifest_sha256') != file_sha256(args.manifest):
    ...
    report = {'schema': 'pusht_failure_audit_v1', 'scope':
              ...
              'manifest_sha256': file_sha256(args.manifest), 'cases': len(cases),
