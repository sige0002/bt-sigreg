...
def build_model():
    ...
    cfg.model._target_='mylewm.algorithms.libero_model.TwoViewJEPA'
...
def main():
    ...
    if args.command=='prepare': prepare(args.dataset,args.manifest)
        ...
        args.adapter_sources={str(f.relative_to(ROOT)):common.file_sha256(f) for f in
            source_files()}
        common.train(args, adapter=common.TrainingAdapter(
            dataset=LiberoClips, preprocess=preprocess, build_model=build_model))
