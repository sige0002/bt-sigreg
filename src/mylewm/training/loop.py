...
from mylewm.algorithms.bt_sigreg import BTSIGReg, add_bt_arguments
from mylewm.data.verification import file_sha256, data_fingerprints, training_budget, verify_training_data
from mylewm.training.diagnostics import encoder_gradient_norms,action_diagnostics,transport_statistics
from mylewm.training.state import (UpdateSchedule, add_schedule_arguments, capture_rng,
...
class TrainingAdapter:
    """Domain-specific I/O and model construction for the shared update loop."""
    dataset: Callable
    preprocess: Callable
    build_model: Callable


def train(args, adapter=None):
    ...
    config={**vars(args),'schema':'controlled_training_v2',
            ...
            'source_sha256':{str(p.relative_to(ROOT)):file_sha256(p) for p in
                [*source_files(),ROOT/'lewm/jepa.py',ROOT/'lewm/module.py',ROOT/'lewm/config/train/model/lewm.yaml']}}
    ...
    if args.resume:
        ...
        if file_sha256(args.output/'initialization.pt')!=state['initialization_sha256']:
    ...
    with (args.output/'metrics.jsonl').open('a') as log:
        for step,batch in enumerate(loader,start_step+1):
            ...
            x,a=adapter.preprocess(batch,m,device)
            ...
            if step%args.save_every==0 or step==args.steps:
                ...
                with isolated_rng(999),torch.no_grad():
                    for vb in val:
                        vx,va=adapter.preprocess(vb,m,device)
                        with torch.autocast(device_type=device,dtype=torch.bfloat16,enabled=device=='cuda'):
